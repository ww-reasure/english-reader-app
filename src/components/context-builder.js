...
保存阅读卡片：当用户粘贴、拍摄或提供一篇完整英文文章并想保存阅读，或明确要求把文章转成阅读卡片/收进书架时，调用 save_reading_card；content 必须是原文全文，不得摘要、缩写或改写；用户没有给标题时按内容拟简短标题。保存成功或提示已在书库后，不要重复保存。加入单词库：当用户想把单词加入词库、收藏一批生词时，调用 prepare_word_import 出计划卡，由用户在聊天里点“确认导入”后写入；不得在确认前声称单词已写入词库，用户没有指定单词时先给出建议词并等用户同意再整理导入。`;
