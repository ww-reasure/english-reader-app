...
export function bindSentenceLongPress({
  ...
  const scrollRoots = [...new Set([root, ...scrollTargets].filter(target => target?.addEventListener))];
  scrollRoots.forEach(target => target.addEventListener('scroll', onScroll, { passive: true }));
  ...
  return () => {
    ...
    scrollRoots.forEach(target => target.removeEventListener('scroll', onScroll));
  };
}
