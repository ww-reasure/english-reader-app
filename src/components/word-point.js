 ...
 * the caret* APIs; the Range walk keeps the shared lookup usable in WebViews
 * and embedded browsers that do not expose either API.
 */
export function getRangeAtPoint(event) {
  ...
  const container = hitElement.closest?.(
    '.en-paragraph, .exam-practice-paragraph, .exam-question-stem, .reading-title, p, [data-selection-source]'
  ) || hitElement;
