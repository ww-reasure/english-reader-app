...
// main 线不含真题训练模块：以下为 exam-context 的最小格式化存根，
// 仅在空 exam 事实下保持 schema 字段兼容（不会被真实真题数据触达）。
const examDisplayName = examId => examId || '';
const resolveExamIdForBank = () => null;
const unitLabel = unit => unit?.displayTitle || '';
