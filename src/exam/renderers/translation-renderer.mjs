...
export const translationRenderer = {
  ...
  renderQuestion(question, { response, unit } = {}) {
    ...
    return `
      <div class="exam-translation-question">
        ...
        <label class="exam-translation-input-label" for="examTranslationInput">${unit?.direction === 'zh_to_en' ? '英文译文' : '中文译文'}</label>
        <textarea id="examTranslationInput" class="exam-translation-input" data-translation-input rows="7" placeholder="${unit?.direction === 'zh_to_en' ? '请输入你的英文译文' : '请输入你的中文译文'}">${esc(response?.value?.text || '')}</textarea>
      </div>`;
  },
  ...
  resultDetailHtml(question, response, context) {
    ...
    const zhToEn = context?.unit?.direction === 'zh_to_en';
    const fields = [];
    fields.push(`<section class="exam-translation-result-section" data-selection-source="translation_source"><h4>${zhToEn ? '原文（中文）' : '原文'}</h4><p>${esc(question.sourceText)}</p></section>`);
    fields.push(`<section class="exam-translation-result-section" data-selection-source="user_translation"><h4>${zhToEn ? '我的译文（英文）' : '我的译文'}</h4><p>${esc(user).replace(/\n/g, '<br>')}</p></section>`);
    if (question.referenceTranslation) fields.push(`<section class="exam-translation-result-section" data-selection-source="reference_translation"><h4>${zhToEn ? '参考译文（英文）' : '参考译文'}</h4><p>${esc(question.referenceTranslation).replace(/\n/g, '<br>')}</p></section>`);
