...
export const ROUTES = Object.freeze([
  ...
  route({
    routeKey: 'exam-home',
    exportName: 'ExamHomeView',
    match: hash => hash === '#/exam',
    cachePolicy: 'keep-alive',
    load: () => import('./views/exam-home.js')
  }),
  route({
    routeKey: 'exam-review',
    exportName: 'ExamReviewView',
    match: hash => hash === '#/exam/review',
    load: () => import('./views/exam-review.js')
  }),
  route({
    routeKey: 'exam-history',
    exportName: 'ExamHistoryView',
    match: hash => hash === '#/exam/history',
    load: () => import('./views/exam-history.js')
  }),
  route({
    routeKey: 'exam-catalog',
    exportName: 'ExamCatalogView',
    match: hash => /^#\/exam\/catalog\/[^/]+$/.test(hash),
    getArgs: hash => [safeDecode(stripQuery(hash).match(/^#\/exam\/catalog\/([^/]+)$/)?.[1])],
    load: () => import('./views/exam-catalog.js')
  }),
  route({
    routeKey: 'exam-practice',
    exportName: 'ExamPracticeView',
    match: hash => /^#\/exam\/practice\/[^/]+(?:\/explanation)?$/.test(hash),
    getArgs: hash => {
      const [, attemptId, mode] = stripQuery(hash).match(/^#\/exam\/practice\/([^/]+)(?:\/(explanation))?$/) || [];
      return [safeDecode(attemptId), mode || null];
    },
    load: () => import('./views/exam-practice.js')
  }),
  route({
    routeKey: 'exam-result',
    exportName: 'ExamResultView',
    match: hash => /^#\/exam\/result\/[^/]+$/.test(hash),
    getArgs: hash => [safeDecode(stripQuery(hash).split('/').pop())],
    load: () => import('./views/exam-result.js')
  }),
  route({
    routeKey: 'not-found',
    exportName: 'NotFoundView',
    match: hash => /^#\/reading\/?$/.test(hash)
      || /^#\/exam\/catalog\/?$/.test(hash)
      || /^#\/exam\/practice\/?$/.test(hash)
      || /^#\/exam\/result\/?$/.test(hash),
    load: notFoundLoad
  }),
  route({
    routeKey: 'profile',
