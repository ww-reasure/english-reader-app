...
test('DeepSeek vision chat release keeps semantic version 2.0.0 and increments Android versionCode to 45', () => {
  assert.match(gradleSource, /versionCode\s+45/);
  assert.match(gradleSource, /versionName\s+["']2\.0\.0["']/);
  assert.equal(versionManifest.versionCode, 45);
