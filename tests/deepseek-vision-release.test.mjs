...
test('private QA release keeps semantic version 2.0.0 and increments Android versionCode to 48', () => {
  assert.match(gradleSource, /versionCode\s+48/);
  assert.match(gradleSource, /versionName\s+["']2\.0\.0["']/);
  assert.equal(versionManifest.versionCode, 48);
