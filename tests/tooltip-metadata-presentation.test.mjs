...
test('places exam and frequency markers beside the word instead of below the definitions', () => {
  const markup = renderTooltipWordBadges({
    ...
    freqLevel: 'high',
    examCorpus: {
      cet4: { priorityTier: 'frequent', priorityLabel: '真题常考' }
    }
  }, value => String(value), 'cet4');

  assert.match(markup, /^<span class="tooltip-word-badge tooltip-word-badge--exam-priority exam-cet4">四级 · 真题常考<\/span><span class="tooltip-word-badge tooltip-word-badge--freq freq-high">通用高频<\/span>$/);
...
test('does not add a low-value marker when the frequency band is unknown', () => {
  const markup = renderTooltipWordBadges({
    examLevels: ['kaoyan-general'],
    freqLevel: 'unknown'
  }, value => String(value), 'kaoyan1');

  assert.match(markup, /考研词表/);
