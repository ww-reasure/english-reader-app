...
test('shared word context resolves an abbreviation-aware sentence in passage markup', () => {
  const originalDocument = globalThis.document;
  const originalNode = globalThis.Node;
  const originalNodeFilter = globalThis.NodeFilter;
  const textNode = { nodeType: 3, textContent: 'Dr. Lee stayed. Target word appears here.', parentElement: null };
  const block = {
    classList: { contains: name => name === 'reading-paragraph' },
    textContent: textNode.textContent,
    closest: selector => selector.includes('.en-paragraph') ? block : null
  };
...
test('guide lookup is isolated, keyboard accessible and fully disposed', async () => {
