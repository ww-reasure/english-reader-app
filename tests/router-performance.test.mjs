...
test('route resolver preserves legacy hash precedence and decoded parameters', async () => {
  ...
  assert.equal(resolveRoute('#/exam/catalog/reading_mcq').routeKey, 'exam-catalog');
  assert.deepEqual(resolveRoute('#/exam/catalog/reading%5Fmcq').args, ['reading_mcq']);
  assert.equal(resolveRoute('#/exam/practice/attempt%2F1/explanation').routeKey, 'exam-practice');
  assert.deepEqual(resolveRoute('#/exam/practice/attempt%2F1/explanation').args, ['attempt/1', 'explanation']);
  assert.equal(resolveRoute('#/exam/result/attempt%2F1').routeKey, 'exam-result');
  assert.deepEqual(resolveRoute('#/exam/result/attempt%2F1').args, ['attempt/1']);
...
test('parameter routes strip the query before decoding and survive invalid encoding', async () => {
  ...
  assert.deepEqual(resolveRoute('#/exam/result/attempt%2F1?from=history').args, ['attempt/1']);
  assert.equal(resolveRoute('#/exam/catalog/reading_mcq?sort=new').args[0], 'reading_mcq');
  assert.deepEqual(resolveRoute('#/exam/practice/attempt%2F1/explanation?x=1').args, ['attempt/1', 'explanation']);
  assert.doesNotThrow(() => resolveRoute('#/exam/result/%ZZ'));
  assert.doesNotThrow(() => resolveRoute('#/exam/catalog/%ZZ'));
  assert.equal(resolveRoute('#/exam/result/%ZZ').args[0], '%ZZ');
});

test('parameter routes without an id fall to an explicit not-found shell', async () => {
  const { resolveRoute } = await import('../src/router-routes.mjs');

  for (const hash of ['#/reading/', '#/reading', '#/exam/catalog/', '#/exam/catalog', '#/exam/practice/', '#/exam/practice', '#/exam/result/', '#/exam/result']) {
    assert.equal(resolveRoute(hash).routeKey, 'not-found', `${hash} must not enter a business view`);
  }
  assert.equal(resolveRoute('#/reading/42').routeKey, 'reading');
  assert.equal(resolveRoute('#/exam/catalog/reading_mcq').routeKey, 'exam-catalog');
  assert.equal(resolveRoute('#/exam/practice/attempt-1').routeKey, 'exam-practice');
  assert.equal(resolveRoute('#/exam/result/attempt-1').routeKey, 'exam-result');
});

test('the not-found route renders a recoverable shell', async () => {
  const { resolveRoute } = await import('../src/router-routes.mjs');
  const route = resolveRoute('#/exam/result/');
...
test('navigation stage evidence is privacy safe, ordered, and uses the injected monotonic clock', async () => {
  ...
  const navigation = controller.navigate('#/exam/result/attempt%2F1?from=history');
  await navigation;
  ...
  assert.ok(payloads.every(payload => payload.routeKey === 'exam-result'), 'only the route key is recorded');
...
test('core route warmup and data preload are promise-deduplicated', async () => {
  ...
  await warmed;
  assert.deepEqual(warmedHashes, ['#/history', '#/vocab', '#/reading-list', '#/flashcard', '#/exam', '#/profile']);
});
